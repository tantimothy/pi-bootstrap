#!/bin/bash
# Opens a terminal attached to `iftop` inside the nettools container, live
# per-connection bandwidth on NETTOOLS_INTERFACE. Meant to be invoked from a
...
"$DOCKER" exec -it "$CONTAINER_NAME" iftop -i "$IFACE"
