...
3. **Host Network Interface Configuration (optional, handled automatically by `run.sh`):** if `NETWORK_STATIC_IPS` is set in `.env` (e.g. `eth0:192.168.1.75 wlan0:192.168.1.77`) and `NETWORK_GATEWAY` is set (e.g. `192.168.1.1`), `run.sh` patches *only* the addressing/gateway/nameservers fields of each named interface's existing netplan config — WiFi SSID/password, NetworkManager UUIDs, and everything else already there are left completely untouched. Leave an interface's IP blank (`wlan0:`) to put it on DHCP instead. Applied via `netplan try`, which auto-reverts within ~120 seconds unless you confirm it — so a mistake here can't lock you out of SSH permanently. Idempotent: only actually re-applies netplan if something changed, so routine redeploys don't re-prompt for confirmation. Leave `NETWORK_STATIC_IPS` unset entirely to skip this and manage networking yourself.
