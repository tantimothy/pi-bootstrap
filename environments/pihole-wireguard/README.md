...
3. **Host Network Interface Configuration (optional, handled automatically by `run.sh`):** if `NETWORK_STATIC_IPS` is set in `.env` (e.g. `eth0:192.168.1.75 wlan0:192.168.1.77`) and `NETWORK_GATEWAY` is set (e.g. `192.168.1.1`), `run.sh` patches *only* the addressing/gateway/nameservers fields of each named interface's existing netplan config — WiFi SSID/password, NetworkManager UUIDs, and everything else already there are left completely untouched. Leave an interface's IP blank (`wlan0:`) to put it on DHCP instead. Leave `NETWORK_STATIC_IPS` unset entirely to skip this and manage networking yourself.

   Two independent safety layers, since this touches live network config: (1) nothing is written to `/etc/netplan` at all until `run.sh` shows you the exact changes and you type `y` to confirm — **default is no** if you just press Enter — and (2) even after confirming, it's applied via `netplan try`, which auto-reverts within ~120 seconds unless separately confirmed again, so a config that looks fine but doesn't actually work still can't lock you out of SSH permanently. Idempotent: if nothing needs to change, this is a silent no-op on routine redeploys — no prompt at all.

4. **Host DNS Resilience (handled automatically by `run.sh`):** since this Pi ends up being its own DNS resolver, `run.sh` configures two host-level fixes on every run (both idempotent — safe to re-run, and only ever act once):
...
The official `ntop/ntopng` image publishes ARM builds, but this hasn't been verified on real Raspberry Pi hardware as part of this repo — if it doesn't come up cleanly on your Pi's specific architecture, `docker logs -f ntopng` is the first thing to check.
