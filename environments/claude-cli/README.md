...
**Fastest path:** `deploy.sh` itself has a **"New Claude CLI Instance..."** action (`new-instance.sh`) in this environment's own policy menu — pick `claude-cli` from the Environments submenu, and it's listed alongside FAST/STOP/CLEAN/etc. It interactively asks for an instance name, SSH port, and workspace path, then does everything described below for you: copies the folder, writes the new `.env`, registers the copy in `config/environments.yaml`, deploys it, and installs its own desktop entries. Every instance carries its own copy of this script, so any instance offers the same action for spawning the next one. Works over plain SSH — no GUI needed — and the same way on macOS. You can also run it directly: `./environments/claude-cli/new-instance.sh`.
...
This registration actually lives in `~/.claude.json` — **not** under `~/.claude/` itself, a distinction that matters: `~/.claude.json` is a separate file Claude Code writes directly to the home directory, outside the `~/.claude/` directory the `${CONTAINER_NAME:-claude-cli}_claude_home` named volume mounts. Left alone that file would live only on the container's own writable layer and be silently lost on every `CLEAN` rebuild — confirmed as a real, previously-unhandled gap, not a hypothetical one. `docker-compose.yml` mounts `~/.claude.json` directly at that exact path from a bind-mounted host file (`data/claude-json/claude.json`, created by `pre-deploy.sh` — see "Persistent Data Directory" below) — not a symlink into `claude_cli_home`, which an earlier version of this fix tried and which broke the first time `claude mcp add` actually ran: Claude Code writes this file via an atomic temp-file-then-rename, and POSIX `rename()` onto a path that's currently a symlink replaces the symlink itself rather than following it through, so the registration silently started landing on the container's own ephemeral layer again instead of the persisted copy. A real mount point doesn't have that failure mode. (An intermediate version used a Docker *named* volume instead of a bind mount, mounted directly onto this exact file path — that hit a separate bug on at least one Docker implementation, OrbStack, which doesn't reliably auto-detect file-vs-directory type for a fresh volume attached to a single-file destination; a bind mount sidesteps that entirely.) Register it once (over an initial SSH session), and it's available whether you come back over SSH or through `/remote-control` — survives a `CLEAN` redeploy too.
...
| Directory | Contents |
|-----------|---------|
| `data/claude-json/` | `~/.claude.json` — MCP server registrations, onboarding state (see "Connecting to Home Assistant") — deleting `data/claude-json/claude.json` drops any registered MCP servers |

This one's a bind-mounted host directory, not a named Docker volume — `pre-deploy.sh` creates it (with a valid placeholder file) before `docker compose` ever runs. Not tracked in git (see `.gitignore`), so a fresh clone never ships it, and it's excluded when `new-instance.sh` copies this folder for a new instance — a new instance always starts with no MCP registrations of its own, not a copy of an existing one's.
