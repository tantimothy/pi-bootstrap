...
case "$-" in
    *i*)
        ...
        # (reached via `herdr machine add ssh://claude@host:PORT`). A herdr
        ...
        if [ -z "$TMUX" ] && { [ -n "$SSH_TTY" ] || [ "${HERDR_ENV:-}" = "1" ]; }; then
            ...
            # name (identify_agent_in_job), so it has to see `claude` —
            ...
            if [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
                ...
                exec sh -c "claude --continue $MODEL_ARGS || claude $MODEL_ARGS"
            fi
