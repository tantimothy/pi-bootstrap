...
if [ "$DEPLOY_MODE" = "container" ]; then
    ...
    if $DOCKER ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        ...
        # OUTSIDE ~/.claude/ itself via an atomic temp-file-then-rename, so
        # a directory-only mount at ~/.claude never protects it.
        #
        # This is NOT a named volume like claude_home, unlike an earlier
        # version of this fix (and unlike claude-cli's own claude_cli_json
        # volume). Confirmed directly against a real deploy of the sibling
        # nanoclaw-mnemon environment, then isolated with a from-scratch
        # `busybox` image and bare `docker run --mount type=volume,...`
        # reproduction (no sibling mounts, no NanoClaw code involved at
        # all): mounting a *named volume* directly onto a file destination
        # reliably fails here with "source .../merged/root/.claude.json is
        # not directory", regardless of `-v` vs `--mount` syntax, regardless
        # of whether the placeholder file is empty or has real content, and
        # regardless of the destination filename having a leading dot or
        # not. That rules out this being about `-v`'s shorthand parsing, the
        # file being zero-byte, or moby's historical mount-ordering/path-
        # prefix bug (moby#8055) — earlier tempting theories that motivated
        # a `-v`-to-`--mount` conversion which then failed identically under
        # live retest. The actual, confirmed cause: this host's Docker
        # implementation is OrbStack's own reimplementation, not upstream
        # dockerd — `docker version`/`docker build` output shows
        # `docker:orbstack` as the builder — and OrbStack has multiple open
        # upstream reports of exactly this failure class for single-file
        # volume/bind mounts (e.g. orbstack/orbstack#1274, #1485): its
        # volume driver doesn't reliably auto-detect file-vs-directory type
        # the way genuine dockerd's copy-up mechanism does.
        #
        ...
        # already exist as a real file, so there's no type to guess. The
        # host file lives in its own directory under $INSTALL_PATH (not
        # loose at the top level) so it rides along with this environment's
        # own existing `data_dirs` entry in info.yaml, getting backed up /
        # WIPE'd the same way `groups/`+`data/` already do, with no separate
        # Docker-volume bookkeeping needed. An atomic rename() landing on a
        # bind-mounted file is exactly as safe as landing on a named-
        # volume-mounted file (both are VFS mount points the kernel
        # redirects reads/writes/renames through; neither behaves like a
        # symlink, which really would get replaced outright — see
        # claude-cli's own Dockerfile for that specific, previously-
        ...
        # ${CONTAINER_NAME}_claude_home (mounted below at /root/.claude) —
        # this one stays a genuine named volume: it's a *directory*
        # destination, and nothing in the reproduction above showed
        # directory-type volume mounts misbehaving here, only file-type ones.
