...
- **"Choose Claude Model"** (`scripts/choose-model.sh`) picks which model that admin session's `claude` launches with — unset (the default) lets Claude Code pick its own default, or pin one (e.g. `claude-sonnet-5`). Writes `CLAUDE_MODEL` to `.env`; in `container` mode it also recreates the orchestrator container (`docker run -e` only takes effect at container creation, not a plain restart), in `host` mode no restart is needed since `open-claude-session.sh` reads `.env` fresh on every launch. Does **not** affect per-conversation-group agent containers — those pick their own model independently of this environment's admin session (see the next bullet). One consequence of `container` mode's recreate: the base `claude` tmux session dies with the old container, so the *next* connection creates a fresh one with the new model — reconnecting to an already-running base session instead just groups onto it, still on whichever model was active when it was created. `docker exec ${CONTAINER_NAME:-nanoclaw} tmux list-sessions` / `env | grep CLAUDE_MODEL` show what's actually live; don't trust asking `claude` itself "which model are you" — that's the model's own self-report, not a reliable runtime check. Use `/status` inside the session instead.
- **Changing a Telegram/Discord group's own model** is a completely separate mechanism from the admin-session setting above — NanoClaw stores it per-group in its own database, via NanoClaw's own `ncl` CLI (container mode: `docker exec -it nanoclaw bash -lc "cd \$NANOCLAW_INSTALL_PATH && pnpm ncl groups list"`, then `... pnpm ncl groups config update --id <group-id> --model claude-sonnet-4-6` and `... pnpm ncl groups restart --id <group-id>`; host mode: run the same `pnpm ncl ...` commands directly from `$NANOCLAW_INSTALL_PATH`, no `docker exec` wrapper needed). `ncl` is a `pnpm` script alias inside NanoClaw's own source, not a global binary — bare `ncl ...` fails with "command not found" without both the `cd` and the `pnpm` prefix. Same self-report caveat as above: after changing it, check with `pnpm ncl groups config get --id <group-id>` rather than trusting the bot's own answer to "which model are you" — confirmed directly that it can keep reporting the previous model as stale metadata even after the change genuinely took effect.
- **In `container` mode, this admin session's history does NOT survive `CLEAN`/recreation** — `/root/.claude` lives only on the container's own ephemeral writable layer, with no dedicated named volume backing it. It survives a plain `docker stop`/`start`, but any container recreation starts history over from nothing (a deliberate tradeoff — a prior attempt at persisting it via a Docker named volume ran into a host-specific Docker-implementation bug; see `docs/lessons-learned/nanoclaw-mnemon.md`). Either way, `/root/CLAUDE.md` (regenerated on every container start — `container` mode only, see `scripts/entrypoint.sh`) gives even a brand-new session with no history basic facts about its own environment (which container, `$NANOCLAW_INSTALL_PATH`) — Claude Code's own standard project-context mechanism, not tied to conversation memory.
