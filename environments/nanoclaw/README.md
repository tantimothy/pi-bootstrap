...
- **In `container` mode, this admin session's history now survives `CLEAN`/recreation** — `${CONTAINER_NAME:-nanoclaw}_claude_home`, a named volume mounted at `/root/.claude` (see `run.sh`), added specifically for this. If you're upgrading from before this volume existed, history accumulated on an old container's own ephemeral layer is already gone with no way to recover it. Either way, `/root/CLAUDE.md` (regenerated on every container start — `container` mode only, see `scripts/entrypoint.sh`) gives even a brand-new session with no history basic facts about its own environment (which container, `$NANOCLAW_INSTALL_PATH`) — Claude Code's own standard project-context mechanism, not tied to conversation memory.
