...
**A more complete alternative**: [`nvk/llm-wiki`](https://github.com/nvk/llm-wiki) is a separate, actively-maintained, standalone implementation of the same Karpathy pattern — not part of NanoClaw, not a variant of `/add-karpathy-llm-wiki` despite the shared inspiration — with parallel research agents, thesis-driven investigation, archiving, and more. See `GIST-PARITY.md` for the full comparison and the open question about whether it's installable inside NanoClaw's agent container at all.
