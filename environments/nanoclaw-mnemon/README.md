...
**Not sure what "persistent graph memory" actually means in practice?** [`MEMORY-EXPLAINED.md`](./MEMORY-EXPLAINED.md) follows one chat message from arrival to reply — every point mnemon touches it, why it fades over time, and what the retrieval jargon means. It covers the proposed OKF graph wiki the same way, for contrast. Start there if the rest of this README assumes more than you'd like.

**Credit**: the Mnemon profile follows the architecture described in a public gist by GitHub user [VivianBalakrishnan](https://gist.github.com/VivianBalakrishnan/a7d4eec3833baee4971a0ee54b08f322). **See [`GIST-PARITY.md`](./GIST-PARITY.md)** for its implementation analysis — that document now compares this environment against *two* gists by the same author, the one this environment follows and a later "OKF Graph Wiki" spec kept purely as a reference point — and **[`MANUAL-STEPS.md`](./MANUAL-STEPS.md)** for the supported manual equivalent.
...
**Memory storage**: mnemon writes to `/home/node/.claude/mnemon/` inside each agent container (`MNEMON_DATA_DIR`, baked in by `apply_mnemon_patch()`), which is the same bind-mounted `.claude-shared` dir `reload-mnemon.sh` already targets — the real host path is `$NANOCLAW_INSTALL_PATH/data/v2-sessions/<group-id>/.claude-shared/mnemon/data/<store>/mnemon.db` (`<store>` is `default` unless `MNEMON_STORE` is set) — memory is per-group by default (mnemon also supports an optional shared/global read-only store; this environment doesn't configure that, it's mnemon's own default per-agent behavior).

**Inspecting mnemon's data directly**: mnemon's own SQLite schema (confirmed against upstream `internal/store/db.go`, not documented in mnemon's own README) is a table named `insights` — not `memories` — holding `id, content, category, importance, tags, entities, source, access_count, created_at, updated_at, deleted_at, last_accessed_at, embedding, effective_importance` (`tags`/`entities` are JSON columns, not separate tables), plus `edges` (`source_id, target_id, edge_type, weight, metadata, created_at`) and an `oplog` audit table. "Probe Mnemon DB — Summary / Top Insights / Interactive sqlite3 Shell" in this environment's `deploy.sh` menu (`scripts/probe-mnemon-db.sh`) reads this data straight off the host — no need to guess the path or schema by hand. It never opens mnemon's live file directly: mnemon runs its store in WAL mode, so `scripts/probe-mnemon-db.sh`/`scripts/export-mnemon-graph.sh`/`scripts/export-mnemon-pages.sh` all take a hot backup via SQLite's own Online Backup API into a throwaway snapshot first, and query that.

**Visualizing the graph**: two options, same discovery/snapshot approach as the probe actions above, both landing in this environment's own gitignored `exports/` (regenerated on demand):
- "Export Mnemon Graph as Interactive HTML (Mnemon profile)" (`scripts/export-mnemon-graph.sh` + `export-mnemon-graph.py`, pyvis) renders a group's insights/edges as a force-directed HTML network and opens it in a browser. Readable for a small store; confirmed live that a real store (353 insights / 4262 edges) renders as an indecipherable tangle at that density. Installs `pyvis` into a private, gitignored venv (`.venv-mnemon-graph`, created on first use) rather than the system/user Python — confirmed live that Homebrew's Python on macOS (PEP 668) refuses a bare `pip install`, `--user` included, without one — gated behind an explicit y/N prompt.
- "Export Mnemon Graph as Navigable Pages (Mnemon profile)" (`scripts/export-mnemon-pages.sh` + `export-mnemon-pages.py`, pure stdlib — no pyvis/venv needed) renders a plain static site instead: a filterable/sortable index plus one page per insight, with its outgoing/incoming edges as real links grouped by edge type. Scales to dense graphs by design — you navigate by following edges rather than untangling a force-directed layout.
...
**This check itself runs on the host, not inside a container** — so if `MNEMON_EMBED_ENDPOINT` uses `host.docker.internal` (the default, meant for the *container's own* runtime reachability, not this script's), `ensure_ollama_ready()` transparently probes `localhost` instead: that hostname is only guaranteed to resolve inside a container via Docker's own embedded DNS, not from a bare host shell. You'll still see `host.docker.internal` in this function's own log output (that's still the value the container itself uses) even though the actual reachability check happens against `localhost` behind the scenes.

**That rewrite is correct and was also a blind spot, so the check no longer stops there.** Probing `localhost` from the host answers "is Ollama running?" and structurally *cannot* answer "can a container reach it?" — and those diverged on a real install: `host.docker.internal` resolved, inside the container, to an address that refused connections, while the host's own LAN IP was reachable from that same container. Every host-side check passed for days while mnemon correctly reported `ollama_available: false`. `ensure_ollama_ready()` now additionally runs `curl` inside a throwaway container against the endpoint agents actually use (with `--noproxy '*'`, since a host-side proxy will otherwise return 200 for a container that has no direct route at all), and if that fails but the host's LAN IP works, it prints the exact `MNEMON_EMBED_ENDPOINT` to switch to. The result is recorded in `pi-bootstrap-patches.md` too, since the admin session inside the container cannot run any of these host-side commands itself.
...
### Optional Codex provider — headless CLEAN, manual OAuth later

Set `NANOCLAW_INSTALL_CODEX=true` before `CLEAN` to install OpenAI Codex as an
additional NanoClaw agent provider. Codex preparation adds no interactive
step to CLEAN: it applies NanoClaw's own current `/add-codex` provider
payload through upstream's directive engine, adds upstream's pinned
`@openai/codex` CLI to the agent-image manifest, and rebuilds the host and
agent images. It does **not** start OAuth, put a token in this environment's
`.env`, change the default provider, or switch an existing group. A completely
fresh NanoClaw installation still has its existing interactive first-run
wizard; this option does not attempt to make that separate bootstrap headless.

After CLEAN, choose **"Manage Codex Provider"** from this environment's
`deploy.sh` action menu. Its operator menu can:

- show whether the provider source, CLI manifest, and agent-image executable
  are present, plus the current group assignments;
- install/refresh the provider without authenticating (a repair path that
  does not require another CLEAN);
- launch NanoClaw's vault-backed ChatGPT subscription/API-key authentication
  walkthrough;
- choose Claude or Codex for one group and restart it;
- set the default provider for groups created later, leaving existing groups
  unchanged;
- restart a selected group or show recent provider-related errors.

Authentication is intentionally a separate manual step because ChatGPT OAuth
needs a browser or device-code handoff. NanoClaw stores the resulting
credential in OneCLI's vault; this environment never copies
...
needs those automatic Mnemon recall/remember hooks.
...
Yes — two separate upstream NanoClaw skills do this, distinct from both mnemon's own embeddings (above) and from each other.

**`/add-ollama-tool` — Claude keeps orchestrating; Ollama becomes a callable tool. Applied automatically, every deploy, both `NANOCLAW_SETUP` profiles.** Unlike everything else on this page, you don't need to run this skill yourself — `run.sh`'s own `apply_ollama_tool_patch()` (the same idempotent text-splice mechanism `apply_mnemon_patch()`/`apply_media_tools_patch()` use, written directly from NanoClaw's real source rather than the skill's own shipped output, which has a known bug it bakes the fix for from the start — `ollama-env.ts` never actually reading `.env`) wires it into every agent-group container's MCP config on every deploy. Registers an MCP server exposing `ollama_list_models` and `ollama_generate`, plus opt-in admin tools (`ollama_pull_model`, `ollama_delete_model`, `ollama_show_model`, `ollama_list_running`, gated behind `.env`'s `OLLAMA_ADMIN_TOOLS=true`). Claude remains the one holding the conversation; Ollama is just another tool it can reach for, the same way it reaches for mnemon or any other MCP tool. See `.env.example`'s `OLLAMA_ADMIN_TOOLS`/`OLLAMA_NO_PROXY_OVERRIDE` comments for the two settings that affect it, and `$NANOCLAW_INSTALL_PATH/pi-bootstrap-patches.md` inside a deployed container for this (and every other) patch's live applied/skipped status — pi-bootstrap tracks NanoClaw's own `main` branch rather than pinning a commit, so that file is the way to notice if an upstream change ever breaks this patch's anchors.

Still opt-in and manual, unchanged from before:
...
**Which model that session's `claude` launches with**: unset (the default) lets Claude Code pick its own default model. Pin one via "Choose Claude Model" in `deploy.sh`'s menu (`scripts/choose-model.sh` — writes `CLAUDE_MODEL` to `.env` and recreates the container, since `docker run -e` only takes effect at container creation) or by editing `CLAUDE_MODEL` in `.env` directly. **Only affects this admin session** — see "Changing a Telegram/Discord Group's Own Model" below for NanoClaw's own per-group setting, a completely separate mechanism. One consequence of "recreates the container" worth knowing: the base `claude` tmux session dies with the old container, so the *next* connection creates a fresh one with the new model — but if you reconnect to an *already-running* base session (grouped onto it, not creating it), you're still talking to whichever model was active when that session was created, regardless of what `.env` currently says. `docker exec ${CONTAINER_NAME:-nanoclaw-mnemon} tmux list-sessions` shows whether a base session is already up; `docker exec ${CONTAINER_NAME:-nanoclaw-mnemon} env | grep CLAUDE_MODEL` shows what's actually baked into the running container. Don't trust asking `claude` itself "which model are you" to check this — a model's own self-report is not a reliable way to verify what's actually running; use `/status` inside the session instead, or the two commands above from the host.

**Changing a Telegram/Discord Group's Own Model**: entirely separate from the admin-session setting above, and not something `CLAUDE_MODEL`/"Choose Claude Model" ever touches. NanoClaw stores each group's model in its own database, set via NanoClaw's own `ncl` CLI:
...
`ncl` is a `pnpm` script alias inside NanoClaw's own source (`package.json`'s `"ncl": "tsx src/cli/client.ts"`), not a global binary — bare `ncl ...` from a plain shell fails with "command not found" regardless of correctness; it needs both the `cd` into `$NANOCLAW_INSTALL_PATH` and the `pnpm` prefix. The change takes effect on that group's own restart (not the orchestrator's), and covers every Telegram/Discord group wired to the same agent group at once. Same self-report caveat as above applies here too: after changing it, don't trust the bot's own "which model are you" answer — confirmed directly that it can keep reporting the *previous* model as stale metadata even after the change genuinely took effect; check with `docker exec -it nanoclaw-mnemon bash -lc "cd \$NANOCLAW_INSTALL_PATH && pnpm ncl groups config get --id <group-id>"` instead.

**This session's history is NOT persisted** — `/root/.claude` lives only on the container's own ephemeral writable layer, with no dedicated named volume backing it. It survives a plain `docker stop`/`start`, but any container *recreation* (`CLEAN`, `TEARDOWN`+redeploy, or "Choose Claude Model"'s own stop+rm+relaunch) starts that history over from nothing. This is a deliberate tradeoff, not an oversight — a prior attempt at persisting it via a Docker named volume ran into a host-specific Docker-implementation bug (see `docs/lessons-learned/nanoclaw-mnemon.md`) and wasn't worth the added fragility for something you can already treat as disposable.

**A fresh session (no history to draw on) still knows basic facts about its own environment** — `/root/CLAUDE.md`, regenerated on every container start (see `scripts/entrypoint.sh`), tells it which container it's running in, `$NANOCLAW_INSTALL_PATH`, and that it's a separate conversation from NanoClaw's own chat-platform one. This is Claude Code's own standard project-context mechanism (any `CLAUDE.md` in a session's launch directory is read automatically), not something specific to memory/history — it's there regardless of whether `--continue` found anything to resume.
...
- **[`MEMORY-EXPLAINED.md`](./MEMORY-EXPLAINED.md)** — in this directory: a plain-language walkthrough of a single message through mnemon, the same for the proposed OKF graph wiki, and a glossary translating BM25, RRF, embeddings, hop decay, trust tiers and the rest.
...
- **A second, later gist by the same author** — ["OKF Graph Wiki"](https://gist.github.com/VivianBalakrishnan/83d1ea5f929d0ae51bca7fe25129b0d7): a full retrieval spec for a Karpathy-pattern wiki built on Google Cloud's [Open Knowledge Format](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing) (v0.1, June 2026), which evaluates mnemon and rejects it. Not implemented here and not planned. `GIST-PARITY.md`'s "The second gist" section carries its component-by-component status against this environment; [`docs/future-enhancements/okf-graph-wiki.md`](../../docs/future-enhancements/okf-graph-wiki.md) has the full analysis and what building any of it would involve.
