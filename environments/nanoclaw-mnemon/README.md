...
2. Adds `mnemon setup --yes` to `container/entrypoint.sh`, run on every agent container start — bare, not `--target claude-code --global`, since mnemon's own README shows only bare `mnemon setup` (auto-detected) for Claude Code specifically, unlike every other integration target it documents with an explicit `--target <name>`.
...
- **Correction, found on a real live deploy**: `mnemon setup --target claude-code --yes --global` (this environment's own patched invocation at the time) ran without erroring on every container start, but its hook registration specifically was never actually confirmed to land in the settings.json Claude Code reads — and on inspection, a real group's own `~/.claude/settings.json` showed only NanoClaw's own memory hooks, no mnemon ones, after real use. Fixed to bare `mnemon setup --yes` (see "Mnemon Integration" above) to match mnemon's own documented Claude Code invocation exactly — `--target claude-code --global` isn't shown anywhere in mnemon's own docs for this specific integration. Re-verify against a fresh `CLEAN` deploy before trusting this claim again.
