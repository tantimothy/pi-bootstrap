...
**This session's history is NOT persisted separately from `${CONTAINER_NAME:-nanoclaw-mnemon}_claude_home`** (a named volume mounted at `/root/.claude`, added specifically for this — see `run.sh`'s own comment). Every container recreation (`CLEAN`, `TEARDOWN`+redeploy, or "Choose Claude Model"'s own recreate) is survived by that volume now, but if you're upgrading from before this volume existed, whatever history was accumulated on the *old* container's own ephemeral layer is already gone — there's no way to recover it retroactively.
