...
**To actually use it**: set `MNEMON_EMBED_ENDPOINT` in `.env` (see `.env.example`'s own commented-out stub), have an Ollama daemon reachable from wherever the agent sandbox runs (`http://host.docker.internal:11434` on Docker Desktop), `ollama pull nomic-embed-text` on that daemon, then `CLEAN` redeploy — same activation path as bumping `MNEMON_VERSION`. Setting the variable alone does nothing if Ollama isn't actually running and reachable; that prerequisite doesn't get automated by this patch, same as it doesn't for `/add-ollama-tool`.
