...
| [chat-frontends](environments/chat-frontends/) | A small hub of browser-based chat UIs for your host's Ollama — [Open WebUI](https://github.com/open-webui/open-webui) (on by default), [SillyTavern](https://github.com/SillyTavern/SillyTavern) (on by default, character/role-play focused), and [LobeHub](https://github.com/lobehub/lobe-chat) (opt-in — needs its own Postgres). Toggle which ones run via `COMPOSE_PROFILES` in `.env`, no rebuild needed |
