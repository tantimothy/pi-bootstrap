...
| [nanoclaw-mnemon](environments/nanoclaw-mnemon/) | Same as nanoclaw, `container` mode only, with [mnemon](https://github.com/mnemon-dev/mnemon) patched into the agent sandbox for persistent cross-session graph memory. Fully independent install — coexists with plain nanoclaw on the same machine |
