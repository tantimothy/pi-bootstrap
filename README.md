...
| [nanoclaw-mnemon](environments/nanoclaw-mnemon/) | Self-hosted AI assistant — routes WhatsApp/Telegram/Discord/Slack/etc. to isolated per-conversation-group Claude Agent SDK containers. It defaults to Mnemon persistent memory, optional Ollama embeddings, wiki scaffolding, and media tools; set `NANOCLAW_SETUP=plain` for an unpatched plain NanoClaw agent image. |
| [openclaw](environments/openclaw/) | Containerized OpenClaw gateway and Control UI, extended with both Claude CLI and Codex CLI. OpenClaw, provider/auth state, both CLI logins, and the shared agent workspace persist across rebuilds. |
...
**Linux only** — `.desktop` files, `~/.local/share/applications`, and the `xdg-desktop-menu` submenu machinery this relies on are all XDG/Linux desktop-environment concepts with no macOS equivalent. On macOS, `./install-desktop-entries.sh` detects this and skips cleanly with a one-line message — nothing is written to `~/Desktop` or anywhere else. If you're running an environment (e.g. `nanoclaw-mnemon`) on a Mac, just run `./deploy.sh` directly, or add it to your Dock/Login Items yourself; there's no equivalent auto-generated launcher on macOS today.
...
| nanoclaw-mnemon | The `nanoclaw-mnemon` container exists (both `mnemon` and `plain` setup profiles are container-only) |
...
| `REBUILD` | Same build-before-swap ordering as `CLEAN`, cheaper: a normal cached build, so only layers whose own instructions changed are replayed — use for a script/config-only edit, not a package-list change. Available for every `docker-compose.yml`/`Dockerfile` environment automatically (implemented once in `lib/deploy-lib.sh`); a custom `run.sh` environment only offers it once that script implements it itself (`kali-pentest`, `metasploit` currently do) | Reuse Docker's layer cache; no `--no-cache`, no pull |
...
For environments driven by `docker-compose.yml`/`Dockerfile` (no `run.sh`), every `FAST`/`STOP`/`TEARDOWN`/`CLEAN` run is recorded to `environments/<env>/logs/<POLICY>-<timestamp>.log`, and likewise for `WIPE` on any environment — the full session, not just a `stdout` capture, via `script`, a real pty recorder. On failure, `deploy.sh` prints the last 30 lines of that log straight to the screen (plus its full path) before returning to the menu, so a fast-scrolling build/install failure doesn't just vanish before you can read it. These logs are gitignored (`environments/*/logs/`) — purely local troubleshooting artifacts, never committed. `INFO` is deliberately **not** logged — it's a read-only display re-run on every menu visit, so a durable "session" record of it would just fill `logs/` with noise never worth reviewing.

Environments with their own `run.sh` (e.g. `nanoclaw-mnemon`, `kali-pentest`) are **not** wrapped by `deploy_environment()`'s own `script`-based logging — stacking `script`'s pty on top of a nested interactive handoff (a `docker exec -it` reattach, an in-container `dialog` menu) was tried and broke both live rendering and keyboard input on a real terminal. Instead, each `run.sh` logs *itself*: it calls `lib/deploy-lib.sh`'s `_selflog_start` right after its own `SCRIPT_DIR`/`POLICY` are known (redirecting its own stdout/stderr through `tee` into the same `environments/<env>/logs/<POLICY>-<timestamp>.log` convention, via process substitution rather than a separate `script` process), then `_selflog_stop` immediately before any interactive handoff, so that part runs completely raw and unlogged — exactly as if `_selflog_start` had never been called. A `run.sh` with no interactive attach of its own at all (`legion`, `ollama`, `mac-terminal-setup`, ...) never needs to call `_selflog_stop` — its whole run is captured. Either way, `deploy.sh` resets the terminal (`stty sane` + drains any stray buffered input) before its next prompt, so a sub-program that exits without restoring the tty can't cause the menu to silently swallow your next keypress or flash by before you can read the result.
...
|---|---|---|---|
| Calls a local `Dockerfile` directly | `[run.sh + Dockerfile]` | `dragonos-sdr`, `kali-pentest`, `nanoclaw-mnemon` | Builds/runs its own single container with an interactive attach/reattach state machine, plus `--privileged`/`--net=host`/`--device` (`dragonos-sdr`/`kali-pentest`), or a Dockerfile that provides a runtime for an external installer to run inside rather than building the app itself (`nanoclaw-mnemon` — see below) |
...
| Clones and delegates to a 3rd-party repo | `[run.sh: 3rd-party repo]` | `internet-pi` | No local `Dockerfile`/`docker-compose.yml` at all — clones an external project and hands off to its own installer (`internet-pi`'s `ansible-playbook`) |
...
`nanoclaw-mnemon` uses the first subtype: its local Dockerfile provides the runtime (Node.js, pnpm, `docker` CLI) for NanoClaw's own interactive installer, rather than baking NanoClaw source into the orchestrator image.
...
**What its README must document:** a "⚙️ Why This Needs a Custom `run.sh`" section, listing concretely what it does that Archetype 2 can't — not just "it's complex," but the specific flags/behaviors from the bullet list above, and which subtype it is. See `dragonos-sdr`, `kali-pentest`, `nanoclaw-mnemon`, `pi-barebones`, and `pihole-wireguard`'s READMEs for real examples of each case. If you can't articulate a concrete reason, use Archetype 2 instead — `portainer` used to have a `run.sh` for no real reason beyond historical inertia, and it was removed once that became clear.
...
- If the environment drops the user into an in-container, multi-tool `dialog`/numbered menu (e.g. `kali-pentest`, `dragonos-sdr`) rather than a single tool, that menu's **first option must be "Info"** — a summary of relevant runtime config, data directory paths, and useful host-side reattach commands, built from values already available inside the container (env vars, mounted paths). `info.yaml` isn't copied into any image, so `deploy.sh`'s outer `INFO` policy (`lib/run-info.sh`) is otherwise completely unreachable once you're attached inside that menu. Like the outer `INFO` screen (`lib/info-lib.sh`), format it with color (bold headers, colored paths — see `lib/info-lib.sh`'s `_color`/`_C_*` for the pattern, gated on `[ -t 1 ]` so a non-interactive `curl | bash` deploy or a redirected/logged run never gets raw escape codes) and pipe it through `less -r` rather than a plain `cat` + `read -p` — -r so color codes survive paging instead of showing as literal garbage.
- **An in-container `dialog` menu has to `export ESCDELAY` itself.** `dialog` is an ncurses program, and ncurses waits `ESCDELAY` milliseconds after a lone Escape byte (0x1b) to find out whether it is really the start of an arrow key or other `\e[...` sequence. The default is a full **1000**, so Esc in the menu looks hung for a second while Ctrl-C — an unambiguous single byte the tty turns straight into SIGINT, with nothing to disambiguate — answers instantly. `lib/locale-lib.sh` sets it to 25 for everything that runs on the host, but nothing under `lib/` is copied into an image and the host's environment does not cross the container boundary, so each in-container menu needs its own `export ESCDELAY=${ESCDELAY:-25}` near the top (`kali-pentest`'s `entrypoint.sh`, `dragonos-sdr`'s generated `sdr-menu.sh`). Not `0` — over ssh the bytes of a real `\e[A` can split across reads, and a zero timeout turns every arrow key into Escape followed by a literal `[A`. The other half of the same delay is tmux's `escape-time` (default 500), now set in every `.tmux.conf` this repo deploys — `mac-terminal-setup`'s and `pi-barebones`' on the host, plus the in-container ones for `aider`, `claude-cli`, `codex-cli` and `nanoclaw-mnemon`, where an inner tmux stacks its own wait on top of the host's. Lowering any one layer leaves the others' waits fully intact.
...
**Only add an `info.sh` override** if the environment needs something `info.yaml` genuinely can't express as static data — for example, `internet-pi`'s `PIHOLE_ENABLE`/`MONITORING_ENABLE` flags deciding which web UIs even exist. Call `_load_info_yaml` first for everything that *is* static, override just the one thing that varies, then call `run_info` directly — see `internet-pi/info.sh` as a template. `ACTION` is always one of `list` (terminal + regenerates `post-deploy-info.html`), `delete` (the `WIPE` policy, with a confirmation prompt), `manifest` (machine-readable, used by `backup.sh`), or `list-dirs` (machine-readable `data_dirs` paths only, one per line, used by `deploy.sh`'s generic fallback path) — none of these are something you call yourself.
...
**Only add an `install-desktop.sh` override** for real branching that isn't expressible as static YAML. Call `_load_desktop_entries_yaml` first for everything that *is* static, set `DEPLOYED_CHECK_KIND`/`DEPLOYED_CHECK_VALUE` yourself, then call `run_desktop_install` directly.
...
- **[`docs/lessons-learned/`](docs/lessons-learned)** — one file per
  ...
  first-deploy issues), plus `general.md` for lessons that generalize
  across the whole repo instead of one environment (checking
  `origin/master` for unrelated work before every push, a merged PR's
  branch being done for good, etc.).
